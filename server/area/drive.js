...
async function refreshToken(user) {
    try {
        oauth2Client.setCredentials({
            refresh_token: user.servicesData.get("google")?.refreshToken,
        });
        const tokens = await oauth2Client.getAccessToken();
        user.servicesData.get("google").accessToken = tokens.res.data.access_token;
        user.servicesData.get("google").refreshToken = tokens.res.data.refresh_token;
        user.save();
...
const createFolderAndFile = async (user, folderName, fileName, fileContent) => {
    try {
        const drive = await refreshToken(user);
